...
ENV OUT_FOLDER vfxstack
ENV BUILD_PROCS 7
ENV BUILD_DIR /opt/vfxstack
...
RUN yum install -y epel-release && yum install -y qtwebkit-devel qtwebkit;

# create build dir
RUN mkdir -p $BUILD_DIR;

#----------------------------------------------
# download and install MAYA 2015
#----------------------------------------------

RUN wget http://download.autodesk.com/us/support/files/maya_2015_service_pack_6/Autodesk_Maya_2015_SP6_EN_Linux.tgz -P /tmp
RUN cd /tmp &&\
    mkdir /tmp/maya &&\
    tar -zxvf /tmp/Autodesk_Maya_2015_SP6_EN_Linux.tgz -C /tmp/maya && \
    cd /tmp/maya &&\
    rpm -Uvh *.rpm

ENV MAYA_ROOT /usr/autodesk/maya


# #----------------------------------------------
# # ARNOLD SDK
# #----------------------------------------------
COPY Arnold-4.2.8.0-linux.tgz /tmp/Arnold-4.2.8.0-linux.tgz

RUN cd /tmp && \
    mkdir -p /tmp/Arnold && \
    tar -zxvf Arnold-4.2.8.0-linux.tgz -C /tmp/Arnold

COPY MtoA-1.2.3.1-linux-2015.run /tmp/MtoA-1.2.3.1-linux-2015.run

RUN  chmod +x /tmp/MtoA-1.2.3.1-linux-2015.run && \
     cd /tmp &&\
     bash MtoA-1.2.3.1-linux-2015.run --tar xvf &&\
     unzip /tmp/package.zip -d /tmp/MTOA

ENV ARNOLD_ROOT /tmp/Arnold
# #----------------------------------------------
# # 3DELIGHT
# #----------------------------------------------
...
RUN wget https://bootstrap.pypa.io/ez_setup.py -O - | $BUILD_DIR/bin/python

#----------------------------------------------
# build and install readline 
#----------------------------------------------
RUN wget https://pypi.python.org/packages/source/r/readline/readline-6.2.4.1.tar.gz -P /tmp
RUN cd /tmp && \
 	tar -zxvf /tmp/readline-6.2.4.1.tar.gz && \
    cd /tmp/readline-6.2.4.1 && \
...
RUN cd /tmp/cortex &&\
    ...
    sed -i 's/\[ "-pipe", "-Wall", "-Werror", "-O2", "-DNDEBUG", "-DBOOST_DISABLE_ASSERTS" \]/\[ "-pipe", "-Wall", "-O2", "-DNDEBUG", "-DBOOST_DISABLE_ASSERTS" \]/g' SConstruct &&\
...
# #----------------------------------------------
# # build and install Gaffer (Master)
# #----------------------------------------------
...
# #----------------------------------------------
# # ABC TO ARNOLD AND DEPENDENCIES
# #----------------------------------------------

# get pystring
RUN git clone https://github.com/imageworks/pystring.git /tmp/pystring
ENV LIBDIR $BUILD_DIR
RUN cd /tmp/pystring &&\
    make -j $BUILD_PROCS &&\
    make LIBDIR=$BUILD_DIR/lib install &&\
    cp -R pystring.h $BUILD_DIR/include
...
RUN cd /tmp/jsoncpp &&\
    ...
    cmake \
        ...
        -DARCHIVE_INSTALL_DIR=$BUILD_DIR \
        -G "Unix Makefiles" \
        .. &&\
    make &&\
    make install

RUN hg clone https://efestolab@bitbucket.org/thepilot/abctoarnold /tmp/abcToArnold &&\
    cd /tmp/abcToArnold &&\
    hg update -r ARNOLD_4.2.8.0


ENV MAYA_VERSION 2015
RUN cd /tmp/abcToArnold &&\
    rm -f build &&\
    sed -i 's/SET(Boost_USE_STATIC_LIBS 1)/SET(Boost_USE_STATIC_LIBS 0)/g' CMakeLists.txt &&\
    mkdir build &&\
    cd build &&\
    cmake \
        -DINSTALL_DIR=$BUILD_DIR/abcToArnold \
        -DMAYA_LOCATION=$MAYA_ROOT \
        -DARNOLD_BASE_DIR=/tmp/Arnold \
        -DMTOA_BASE_DIR=/tmp/MTOA \
        -DCMAKE_INSTALL_PREFIX=$BUILD_DIR \
        -DILMBASE_ROOT=$BUILD_DIR \
        -DBOOST_ROOT=$BUILD_DIR \
        -DBOOST_LIBRARYDIR=$BUILD_DIR/lib \
        -DBoost_NO_SYSTEM_PATHS=TRUE \
        -DBoost_NO_BOOST_CMAKE=TRUE \
        -DPYSTRING_DIR=$BUILD_DIR/lib \
        -DPYSTRING_INCLUDE_DIR=$BUILD_DIR \
        .. &&\
    make &&\
    make install
...
# # prepare the output
# #----------------------------------------------
RUN chown -R vfxstack:vfxstack /opt
VOLUME /$OUT_FOLDER
